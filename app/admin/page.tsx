...
import { FaHome, FaChevronRight, FaChevronLeft, FaSearch, FaFilter, FaChevronDown, FaPlus, FaEllipsisH, FaEdit, FaEye, FaCopy, FaArchive, FaImage, FaSave, FaUndo, FaTrash } from 'react-icons/fa';
...
import { PAGES, getPageBySlug, hasStructure, PageDefinition, SectionDefinition } from '@/app/admin/content-structures';
...
export default function AdminPages() {
  ...
  const handleFieldChange = (sIdx: number, fieldKey: string, value: string) => {
    setDraftContent(prev => {
      ...
      section.fields = section.fields.map(f => f.key === fieldKey ? { ...f, value } : f);
      next[sIdx] = section;
      return next;
    });
  };
  ...
  const handleViewPage = () => {
    const url = `${selectedPage.slug}?preview=1`;
  ...
  const handleDeleteListItem = (sIdx: number, iIdx: number) => {
    setDraftContent(prev => {
      ...
      const section = { ...next[sIdx] };
      if (section.list) {
        section.list = { ...section.list, items: section.list.items.filter((_, i) => i !== iIdx) };
      }
      next[sIdx] = section;
      return next;
    });
  ...
  const handleAddListItem = (sIdx: number) => {
    setDraftContent(prev => {
      ...
      if (section.list) {
        ...
        const newItem: any = {};
        Object.keys(template).forEach(k => { newItem[k] = Array.isArray(template[k]) ? [] : ''; });
        section.list = { ...section.list, items: [...section.list.items, newItem] };
      }
      next[sIdx] = section;
      return next;
    });
  ...
  const handleSave = () => {
    ...
    alert('Changes saved successfully! Open "View Page" to see the live preview.');
  };
  ...
  const handleDuplicate = () => {
    const newSlug = `${selectedPage.slug}-copy-${pages.length + 1}`;
    const newPage: PageDefinition = {
      ...
      structure: JSON.parse(JSON.stringify(selectedPage.structure)),
    };
  ...
  const handleArchive = () => {
    setPages(prev => prev.map(p => p.slug === selectedPage.slug ? { ...p, status: 'Archived' } : p));
    setSelectedPage(prev => ({ ...prev, status: 'Archived' }));
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FA] font-sans text-gray-800">
      ...
      <main className="flex-1 ml-64 flex flex-col">
        <AdminHeader isEditMode={isEditMode} onToggleEditMode={() => setIsEditMode(!isEditMode)} />
        ...
        <div className="p-8 flex gap-8 h-[calc(100vh-64px)] overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="mb-6">
              ...
              <div className="flex justify-between items-start">
                ...
                <button onClick={handleAddPage} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-sm">
                  <FaPlus /> Add New Page
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <FilterTab label="All Pages" count={counts.All} active={activeFilter === 'All'} onClick={() => setActiveFilter('All')} />
                <FilterTab label="Published" count={counts.Published} color="green" active={activeFilter === 'Published'} onClick={() => setActiveFilter('Published')} />
                <FilterTab label="Draft" count={counts.Draft} color="yellow" active={activeFilter === 'Draft'} onClick={() => setActiveFilter('Draft')} />
                <FilterTab label="Archived" count={counts.Archived} active={activeFilter === 'Archived'} onClick={() => setActiveFilter('Archived')} />
              </div>
              <div className="flex items-center gap-3">
                <div className="relative">
                  <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs" />
                  <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search pages..." className="pl-8 pr-3 py-1.5 border border-gray-300 rounded-md text-sm" />
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    {filteredPages.length > 0 ? filteredPages.map(page => (
                      <tr key={page.slug} onClick={() => setSelectedPage(page)} className={`cursor-pointer ${selectedPage.slug === page.slug ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                        <td className="px-6 py-4"><input type="checkbox" checked={selectedPage.slug === page.slug} onChange={() => setSelectedPage(page)} /></td>
                        ...
                        <td className="px-6 py-4"><span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${getCategoryColor(page.category)}`}>{page.category}</span></td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            page.status === 'Published' ? 'bg-green-100 text-green-700' :
                            page.status === 'Archived' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'
                          }`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${
                              page.status === 'Published' ? 'bg-green-500' :
                              page.status === 'Archived' ? 'bg-gray-500' : 'bg-yellow-500'
                            }`}></span>
                            {page.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-gray-500 text-xs">{page.updated}</td>
                        <td className="px-6 py-4 text-center"><button className="text-gray-400 hover:text-gray-600"><FaEllipsisH /></button></td>
                      </tr>
                    )) : (
                      <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">No pages found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              <div className="mt-auto border-t border-gray-200 px-6 py-4 flex items-center justify-between bg-white">
                <span className="text-xs text-gray-500">Showing 1–{filteredPages.length} of {pages.length} pages</span>
                <div className="flex items-center gap-2">
                  <button className="p-1 rounded border border-gray-300 text-gray-400" disabled><FaChevronLeft className="text-xs" /></button>
                  <button className="w-6 h-6 rounded bg-blue-600 text-white text-xs">1</button>
                  <button className="p-1 rounded border border-gray-300 text-gray-400" disabled><FaChevronRight className="text-xs" /></button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-96 bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
            <div className="flex border-b border-gray-200">
              <button onClick={() => setActiveTab('details')} className={`flex-1 py-3 text-sm font-medium border-b-2 ${activeTab === 'details' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500'}`}>Page Details</button>
              <button onClick={() => setActiveTab('editor')} className={`flex-1 py-3 text-sm font-medium border-b-2 ${activeTab === 'editor' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500'}`}>Content Editor</button>
            </div>
